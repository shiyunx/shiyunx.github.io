---
layout: post
title: Date Tracker
date: 2024-09-15
description: Automated date tracker in excel
repo:
category: Automations
permalink: /posts/2024-09-15-date-tracker/
---
Below are the steps to automate the daily data tracking:
- Create the list with important columns
- Start date
- End date
- Remaining days
- Status

Important columns
- Add another notification column with today’s date and set the reminder days.

<img src="/assets/images/posts-images/2024-09-15-date-tracker1.png" 
     alt="Date Tracker1" 
     style="width: 50%; max-width: 200px; display: block; margin: 1rem auto;">

Notifications column
- Enter data start date, end date and calculate the remaining days.

<img src="/assets/images/posts-images/2024-09-15-date-tracker2.png" 
     alt="Date Tracker2" 
     style="width: 70%; max-width: 700px; display: block; margin: 1rem auto;">

- Enter the formula in status column

<img src="/assets/images/posts-images/2024-09-15-date-tracker3.png" 
     alt="Date Tracker3" 
     style="width: 70%; max-width: 700px; display: block; margin: 1rem auto;">

Tracking formula
- If the data is huge, it would be more visible to set the status column with colours.
- Go to conditional formatting > manage rules > Set the status colours

<img src="/assets/images/posts-images/2024-09-15-date-tracker4.png" 
     alt="Date Tracker4" 
     style="width: 70%; max-width: 700px; display: block; margin: 1rem auto;">
